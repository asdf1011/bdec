...
import string
import activegrid.tool.findservice
import os
import sys
_ = wx.GetTranslation
...
class TextView(wx.lib.docview.View):
    ...
    def ProcessEvent(self, event):        
        ...
        if id == wx.ID_UNDO:
            ...
            return True
        elif id == activegrid.tool.findservice.FindService.FIND_ID:
            self.OnFind()
            return True
        elif id == activegrid.tool.findservice.FindService.FIND_PREVIOUS_ID:
            self.DoFind(forceFindPrevious = True)
            return True
        elif id == activegrid.tool.findservice.FindService.FIND_NEXT_ID:
            self.DoFind(forceFindNext = True)
            return True
        elif id == activegrid.tool.findservice.FindService.REPLACE_ID:
            self.OnFind(replace = True)
            return True
        elif id == activegrid.tool.findservice.FindService.FINDONE_ID:
            self.DoFind()
            return True
        elif id == activegrid.tool.findservice.FindService.REPLACEONE_ID:
            self.DoFind(replace = True)
            return True
        elif id == activegrid.tool.findservice.FindService.REPLACEALL_ID:
            self.DoFind(replaceAll = True)
            return True
        elif id == activegrid.tool.findservice.FindService.GOTO_LINE_ID:
    ...
    def OnFind(self, replace = False):
        findService = wx.GetApp().GetService(activegrid.tool.findservice.FindService)
        if findService:
    ...
    def DoFind(self, forceFindNext = False, forceFindPrevious = False, replace = False, replaceAll = False):
        ...
        regExp = flags & activegrid.tool.findservice.FindService.FR_REGEXP > 0
        down = flags & wx.FR_DOWN > 0
        wrap = flags & activegrid.tool.findservice.FindService.FR_WRAP > 0
        ...
        if replace:
            ...
            if result > 0:
                ...
                endLoc = startLoc
            elif result == activegrid.tool.findservice.FIND_SYNTAXERROR:
                badSyntax = True
